...
2. **Make Changes**
   ...
   - If you edit `schemas/*.gschema.xml`, run `glib-compile-schemas` on the installed extension schemas (or reinstall via `./install.sh`). The repository/release packages ship XML only and do **not** track `schemas/gschemas.compiled`.
